...
// persistedServiceConfigResponse is a mirror image of
// *structs.ServiceConfigResponse without QueryMeta fields. These should be
// kept in sync.
type persistedServiceConfigResponse struct {
	ProxyConfig     map[string]interface{}
	UpstreamConfigs map[string]map[string]interface{}
	MeshGateway     structs.MeshGatewayConfig `json:",omitempty"`
}

func (r *persistedServiceConfigResponse) ToResponse() *structs.ServiceConfigResponse {
	return &structs.ServiceConfigResponse{
		ProxyConfig:     r.ProxyConfig,
		UpstreamConfigs: r.UpstreamConfigs,
		MeshGateway:     r.MeshGateway,
	}
}
...
func (a *Agent) persistServiceConfig(serviceID string, defaults *structs.ServiceConfigResponse) error {
	...
	wrapped := persistedServiceConfig{
		ServiceID: serviceID,
		Defaults: &persistedServiceConfigResponse{
			ProxyConfig:     defaults.ProxyConfig,
			UpstreamConfigs: defaults.UpstreamConfigs,
			MeshGateway:     defaults.MeshGateway,
		},
	}
...
func (a *Agent) readPersistedServiceConfigs() (map[string]*structs.ServiceConfigResponse, error) {
	...
	for _, fi := range files {
		...
		out[p.ServiceID] = p.Defaults.ToResponse()
	}
...
func (a *Agent) AddServiceAndReplaceChecks(service *structs.NodeService, chkTypes []*structs.CheckType, persist bool, token string, source configSource) error {
	a.stateLock.Lock()
	defer a.stateLock.Unlock()
	return a.addServiceLocked(service, chkTypes, nil, true, persist, true, token, true, source)
}
...
func (a *Agent) AddService(service *structs.NodeService, chkTypes []*structs.CheckType, persist bool, token string, source configSource) error {
	a.stateLock.Lock()
	defer a.stateLock.Unlock()
	return a.addServiceLocked(service, chkTypes, nil, true, persist, true, token, false, source)
}
...
func (a *Agent) addServiceLocked(
	...
	if a.config.EnableCentralServiceConfig {
		return a.serviceManager.AddService(
			service,
			chkTypes,
			previousDefaults,
	...
	return a.addServiceInternal(&addServiceRequest{
		service:               service,
		chkTypes:              chkTypes,
		persistService:        nil,
		persistDefaults:       nil,
		persist:               persist,
		persistServiceConfig:  false,
		token:                 token,
...
func (a *Agent) loadServices(conf *config.RuntimeConfig) error {
	...
	for _, service := range conf.Services {
		...
		err = a.addServiceLocked(
			ns,
			chkTypes,
			persistedServiceConfigs[serviceID],
			false, // waitForCentralConfig: exclusively use cached values
			false, // persist: don't rewrite the file with the same data we just read
			false, // persistServiceConfig: don't rewrite the file with the same data we just read
			service.Token,
			false, // replaceExistingChecks: do default behavior
			ConfigSourceLocal,
		)
		if err != nil {
		...
		if sidecar != nil {
			...
			err = a.addServiceLocked(
				sidecar,
				sidecarChecks,
				persistedServiceConfigs[sidecarServiceID],
				false, // waitForCentralConfig: exclusively use cached values
				false, // persist: don't rewrite the file with the same data we just read
				false, // persistServiceConfig: don't rewrite the file with the same data we just read
				sidecarToken,
